...
from cosmos_predict2.configs.traj_conditioned.defaults.data import register_training_and_val_data_traj_conditioned
from cosmos_predict2.configs.traj_conditioned.defaults.model import register_model_traj_conditioned
...
class Config(config.Config):
    ...
    defaults: List[Any] = attrs.field(
        factory=lambda: [
            ...
            {"scheduler": "constant"}, # NOTE (junjie): transfer use lambdalinear
            {"model": "predict2_video2world_fsdp_2b"}, # NOTE (junjie): transfer use VideoDiffusionModelWithCtrl
            {"callbacks": ["basic"]},
            {"net": None},
            {"ema": None},
            {"checkpoint": None},
    ...
    # NOTE (junjie): decide if we need to use these configs
            # {"net_ctrl": None},
...
def make_config() -> Config:
    ...
    # traj conditional post-training config
    register_training_and_val_data_traj_conditioned()
    register_model_traj_conditioned()
    ...
    import_all_modules_from_package("cosmos_predict2.configs.traj_conditioned.experiment", reload=True)
    return c
