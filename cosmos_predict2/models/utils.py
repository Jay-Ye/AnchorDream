...
## isolated from video2world_model.py
...
def assign_lora_weights(dit, dit_path, device="cuda", dtype=torch.bfloat16):
    ...
    if w is not None and w.shape != dit.traj_embedder.input_proj.weight.shape:
        print(f"Removing traj_embedder.input_proj.weight because it's not the same shape")
        state_dict_dit_compatible.pop("traj_embedder.input_proj.weight", None)

    dit.load_state_dict(state_dict_dit_compatible, strict=False, assign=True)
    dit = dit.to(device=device, dtype=dtype)
    del state_dict, state_dict_dit_compatible

def load_lora_weights(pipe, dit_path, training_config_path, device="cuda", dtype=torch.bfloat16):
    ...
    assign_lora_weights(pipe.dit, dit_path, device=device, dtype=dtype)