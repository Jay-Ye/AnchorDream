...
try:
    from vidar.utils.config import Config, read_config
...
try:
    from custom.visuals.visuals import matrix4d_visuals_metrics
    from custom.wandb import prep_video
except ImportError:
    matrix4d_visuals_metrics = None
    prep_video = None
...
class Predict2ModelManagerConfig:
    dit_path: str = "checkpoints/nvidia/Cosmos-Predict2-2B-Video2World/model-480p-10fps.pt"
    dit_ema_path: str = "checkpoints/nvidia/Cosmos-Predict2-2B-Video2World/model-480p-10fps.pt"
    text_encoder_path: str = "checkpoints/google-t5/t5-11b"
    vae_path: str = "checkpoints/nvidia/Cosmos-Predict2-2B-Video2World/tokenizer/tokenizer.pth"
...
class Predict2Video2WorldModelConfig:
    ...
    control_video_key: str = "control_video"
...
class Predict2Video2WorldModel(ImaginaireModel):
    ...
    def setup_modules(self, config):
        # Set up transforms
        if config.transforms != '':
            self.transforms = GenericModel(read_config(f'custom/config/transforms/{config.transforms}.yaml'))
        else:
            self.transforms = None
        # Set up vae
        if config.vae != '':
            vae = getattr(importlib.import_module(f'custom.vae.{config.vae}'), 'VAE')
            self.vae = vae(
                self.device, 
                config.model_manager_config.vae_path, 
                config.model_manager_config.text_encoder_path,
            )
        else:
            self.vae = None
        # Set up vidar2a4d
        if config.vidar2a4d != '':
            self.vidar2a4d = importlib.import_module(f"custom.vidar2a4d.{config.vidar2a4d}").vidar2a4d_fn
        else:
            self.vidar2a4d = None
        # Set up arch
        if config.arch != '':
            arch = importlib.import_module(f"custom.arch.{config.arch}").recipe_arch
            recipe = Config(
                model=Config(
                    config=Config()
                )
            )
            self.recipe = arch(recipe)
        else:
            self.recipe = None
    ...
    def run_modules(self, data_batch, phase):
        ...
        if 'guidance' in data_batch:
            data_batch['guidance'] = data_batch['guidance'][0].item()
        if self.transforms is not None:
            data_batch['vidar'] = self.transforms.prepare_batch(data_batch['vidar'])
        if self.vidar2a4d is not None:
            data_batch = self.vidar2a4d(data_batch, phase=phase)
        if self.vae is not None:
            data_batch = self.vae.process_matrix4d(data_batch, phase=phase)
        return data_batch

    def training_step(self, data_batch: dict, data_batch_idx: int) -> tuple[dict, torch.Tensor]:
        self.pipe.device = self.device

        # Run data through modules
        data_batch = self.run_modules(data_batch, phase='train')
    ...
    @torch.no_grad()
    def validation_step(
        self, data_batch: dict[str, torch.Tensor], iteration: int, local_path: str,
        ...
        """
        loss = None
        if self.vidar2a4d is None:
            val_output_batch, loss = self.training_step(data_batch.copy(), iteration)
            val_loss_dict = {
                'val_loss/loss': val_output_batch['loss'],
                'val_loss/mse_loss': val_output_batch['mse_loss'],
                'val_loss/edm_loss': val_output_batch['edm_loss'],
            }
            data_batch = self.run_modules(data_batch, phase='val')
            # Get generated videos from pipeline
            output_batch = self.pipe.process(
                data_batch, 
                seed=0, 
                guidance=data_batch['guidance'][0].item() if 'guidance' in data_batch else 7.0
            )
            
            ## log videos to wandb
            if not os.path.exists(f'{local_path}/visuals'):
                os.makedirs(f'{local_path}/visuals', exist_ok=True)

            # Save videos and collect video info for this rank
            rank = torch.distributed.get_rank() if torch.distributed.is_initialized() else 0
            world_size = torch.distributed.get_world_size() if torch.distributed.is_initialized() else 1
            
            # Collect video info for this rank
            rank_video_info = []
            for i in range(output_batch.shape[0]):
                # Convert from [-1,1] to [0,255] uint8
                video = ((output_batch[i].permute(1,2,3,0) + 1) / 2 * 255).clamp(0,255).cpu().numpy().astype('uint8')
                save_path = f'{local_path}/visuals/video_{i}_{rank}.mp4'
                imageio.mimsave(save_path, video, fps=10)
                
                # Store video info for this rank
                video_info = {
                    'key': f'generated_video_{i}_{rank}',
                    'path': save_path,
                    'caption': data_batch['video_name']['video_path'][i] if 'video_name' in data_batch and 'video_path' in data_batch['video_name'] else f'Video {i} from rank {rank}'
                }
                rank_video_info.append(video_info)
            
            # Synchronize all ranks before gathering
            if torch.distributed.is_initialized():
                torch.distributed.barrier()
                # Gather the list of video info from all ranks
                gathered_video_info = [None] * world_size
                torch.distributed.all_gather_object(gathered_video_info, rank_video_info)
            else:
                gathered_video_info = [rank_video_info]
            
            # Create unified videos dictionary (only on rank 0 for wandb logging)
            videos = []
            if rank == 0:  # Only rank 0 creates the wandb videos
                total_videos_collected = 0
                for rank_idx, rank_videos in enumerate(gathered_video_info):
                    if rank_videos is not None:  # Handle None entries
                        for video_info in rank_videos:
                ...
                log.info(f"Collected {total_videos_collected} videos from {world_size} ranks for wandb logging")
            
            output_batch = {'logs': {'videos': videos, **val_loss_dict}}

            return output_batch, loss

        else:
            data_batch = self.run_modules(data_batch, phase='val')
            y0s_pred = self.pipe.process(
                data_batch, seed=0, guidance=data_batch['guidance'] if 'guidance' in data_batch else 7.0,
                return_latents=True, is_negative_prompt=False,
            )

            x0s = data_batch['rgb_dst'].clone()
            x0s[:, :, data_batch['num_conditional_frames']:] = 0
            y0s = data_batch['rgb_dst'].clone()

            output_dict = {
                'x0s':      {'dst': x0s},
                'y0s':      {'dst': y0s},
                'y0s_pred': {'dst': y0s_pred},
            }
            if matrix4d_visuals_metrics is not None and prep_video is not None:
                (nice_videos, metrics_dict) = matrix4d_visuals_metrics(
                    iteration, 'val', data_batch, output_dict, self.vae, self.recipe.model.config, loss,
                    f'{local_path}/visuals', quality_pix=7, quality_lat=8, detail=3, verbosity=1
                )

                logs = {}
                for key1, val1 in nice_videos.items():
                    video = prep_video(key1, val1)
                    for key2, val2 in video.items():
                        logs[f"videos/{key2.upper()}--{data_batch['dl_idx'][0]}--{data_batch['dset_name'][0]}"] = val2
    ...
    def ema_beta(self, iteration: int) -> float:
        ...
        """
        iteration = iteration + self.config.pipe_config.ema.iteration_shift # do we want to update the iteration here?
        if iteration < 1:
            return 0.0
