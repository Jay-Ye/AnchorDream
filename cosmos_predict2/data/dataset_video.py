...
class Dataset(Dataset):
    def __init__(
        ...
        self.is_vidar = dataset_dir.endswith('.yaml')

        if self.is_vidar:
            try:
                from custom.dataset.vidar_dataset import VidarDataset
                self.vidar_dataset = VidarDataset(dataset_dir, num_frames, phase)
            except ImportError:
                raise ImportError("VidarDataset requires the vidar package. The traj_conditioned pipeline does not need this.")
            return None
    ...
    def __str__(self):
        if self.is_vidar:
            return self.vidar_dataset.__str__()
    ...
    def __getitem__(self, index):
        try:
            if self.is_vidar:
                return self.vidar_dataset[index]
