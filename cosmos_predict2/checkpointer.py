...
from custom.utils.utils import get_data_folder, get_resume
...
class Checkpointer:
    ...
    def __init__(self, config_checkpoint: CheckpointConfig, config_job: JobConfig, callbacks: callback.CallBackGroup):
        ...
        self.checkpoint_dir_local = f"{get_data_folder()}/{config_job.path_local}/checkpoints"
        ...
        if self.resume is not None:
            self.resume = get_resume(self.resume)
        self.pretrained = config_checkpoint.pretrained
        if self.pretrained is not None:
            self.pretrained = get_resume(self.pretrained)
        ...
        if distributed.get_rank() == 0:
            ...
            for folder in folders:
                ...
                self.save_thread = threading.Thread(
                    ...
                    daemon=False,
                    args=(state_dict, checkpoint_path, distributed.get_rank(), s3_folder),
                )
    ...
    def _save_worker_local(self, state_dict: dict[str, torch.Tensor], checkpoint_path: str, rank: int = 0, s3_folder: str = None) -> None:
        ...
        try:
            if rank == 0:
                ...
                if s3_folder is not None:
                    # upload to s3
                    item = checkpoint_path.split("/")[-2]
                    s3_path = os.path.join(s3_folder, item, checkpoint_file)
                    subprocess.call(
                        ["aws", "s3", "cp", checkpoint_path, s3_path, "--acl", "bucket-owner-full-control"],
                    )
                    log.success(f"Uploaded checkpoint to S3 at {s3_path}")
                    # if upload to s3, remove after saving
                    os.system(f'rm {checkpoint_path}')
                    log.success(f"Removed checkpoint from local disk {checkpoint_path}")
