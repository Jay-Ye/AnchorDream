...
def save_video_frames(
    tensor: Tensor, save_dir: str, frame_prefix: str = "frame_", quality: int = 85
) -> None:
    """
    Save a video tensor as individual frames

    Args:
        tensor (Tensor): Input tensor with shape (B, C, T, H, W) or (C, T, H, W) in [-1, 1] or [0, 1] range.
            If in [-1, 1] range, it will be automatically converted to [0, 1] range.
        save_dir (str): Directory path to save the frames
        frame_prefix (str): Prefix for frame filenames. Default is "frame_"
        quality (int): JPEG quality (0-100). Default is 85
    """
    # Handle batch dimension if present
    if tensor.ndim == 5:
        tensor = tensor[0]  # Take the first item from the batch
    ...
    frames = (rearrange((tensor.cpu().float().numpy() * 255), "c t h w -> t h w c") + 0.5).astype(np.uint8)
    
    # Save each frame
    for i in range(frames.shape[0]):
        frame = PILImage.fromarray(frames[i])
        frame_path = os.path.join(save_dir, f"{frame_prefix}{i:06d}.jpg")
        easy_io.dump(frame, frame_path, file_format="jpg", format="JPEG", quality=quality)
