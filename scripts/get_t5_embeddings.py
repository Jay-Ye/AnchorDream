...
def main(args) -> None:
    ...
    for meta_filename in tqdm(metas_list):
        ...
        with open(meta_filename, "r") as fp:
            if "LBM" in meta_filename or "lbm" in meta_filename:
                prompt = fp.readlines()
            else:
                prompt = fp.read().strip()

        if isinstance(prompt, list):
            for i, prompt_line in enumerate(prompt):
                print(prompt_line)
                t5_xxl_filename = os.path.join(t5_xxl_dir, f"{prompt_line.replace(' ', '_')}.pickle")
                get_t5_embeddings(prompt_line.strip(), t5_xxl_filename, args.max_length, encoder)
        else:
            get_t5_embeddings(prompt, t5_xxl_filename, args.max_length, encoder)
